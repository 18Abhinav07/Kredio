...
const noSelector = process.argv[3] === 'nosel';

const bytecode = readFileSync(join(ROOT, `contracts/pvm/${contract}/target/ink/${contract}.polkavm`));
console.log('Bytecode size:', bytecode.length, 'bytes');

const selector = noSelector ? '' : '9bae9d5e';
const data = '0x' + bytecode.toString('hex') + selector;
console.log('Selector appended:', !noSelector);
